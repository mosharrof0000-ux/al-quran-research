...
(function(){
  ...
  function addStyles(){
    ...
    const s=document.createElement('style'); s.id='aq-chat-v2-style'; s.textContent=`
      ...
      .aq-thinking-btn{width:100%;border:1px solid #d9b75d;border-radius:18px;min-height:72px;display:flex;align-items:center;gap:14px;padding:12px 16px;background:linear-gradient(135deg,#183f39,#2b2046);color:#fff;cursor:pointer;text-align:left;box-shadow:inset 0 1px #fff2,0 8px 20px #0005;font:inherit}
      .aq-thinking-btn:active{transform:scale(.985)}
      .aq-thinking-btn .aq-ti{font-size:30px}.aq-thinking-btn b{display:block;font-size:19px}.aq-thinking-btn span{font-size:12px;color:#ddd8d0}
  ...
  function addThinkingButton(){
    if(document.getElementById('aq-thinking-btn'))return;
    const home=document.getElementById('home');
    if(!home)return;
    const actions=home.querySelector('.home-actions');
    if(!actions)return;
    const b=document.createElement('button');
    b.id='aq-thinking-btn';
    b.className='aq-thinking-btn';
    b.type='button';
    b.innerHTML='<span class="aq-ti">💡</span><span><b>আমাদের চিন্তা</b><span>গবেষণার ভাবনা, ভবিষ্যৎ পরিকল্পনা ও দর্শন</span></span><span class="arrow" style="margin-left:auto;font-size:30px;color:#e8d58e">›</span>';
    b.addEventListener('click',function(){window.location.href='our-thinking.html'});
  ...
  function watch(){addThinkingButton();bindChatOpenButtons();if(setup())return;setTimeout(watch,300)}
