/* আল-কুরআন গবেষণা — Worker Entry Wrapper v4.5
...
async function directGemini(request,env,origin){
 ...
 for(const model of models){try{const r=await fetch(`https://generativelanguage.googleapis.com/v1beta/models/${model}:generateContent`,{method:'POST',headers:{'Content-Type':'application/json','x-goog-api-key':env.GEMINI_API_KEY},body:JSON.stringify({systemInstruction:{parts:[{text:BRAIN_SYSTEM}]},contents:[{role:'user',parts:[{text:prompt}]}],generationConfig:{maxOutputTokens:4096,temperature:0.1}})});if(!r.ok)continue;const data=await r.json();const answer=data?.candidates?.[0]?.content?.parts?.map(p=>p?.text||'').join('').trim();if(answer){const recordProposal=buildResearchRecordProposal({message,answer,researchProvenance:research.provenance,memoryRecords:memory.records});return json({answer,mode,language:'bn',provider:model,brain_version:BRAIN_VERSION,intent:brain.type,research_context_used:research.used,research_context_ref:research.ref?`${research.ref[0]}:${research.ref[1]}`:null,research_memory_used:memory.used,research_memory_records:memory.records.map(r=>({record_id:r.record_id,version:r.version,status:r.status,ayah_id:r.scope?.ayah_id||null})),project_context_loaded:Boolean(projectContext),research_provenance:research.provenance,research_record_proposal:recordProposal},200,origin);}}catch{}}
 return null;
}
