/* আল-কুরআন গবেষণা — Worker Entry Wrapper v4.6
...
import { RESEARCH_OUTPUT_STYLE, RESEARCH_OUTPUT_STYLE_VERSION } from './research-output-style.js';
import { loadProjectContext } from './research-project-context.js';
import { loadResearchMemory } from './research-memory.js';
import { buildResearchRecordProposal } from './research-record-gate.js';
import { handlePrivateResearch } from './private-research.js';
import { classifyResearchQuery } from './research-intelligence-router.js';
import { handleApiGateway } from './api-gateway-v1.js';
...
async function diagnostic(request,env){
 ...
 const out={ok:true,service:'al-quran-research-diagnostic',worker_runtime:'reachable',brain_version:BRAIN_VERSION,research_output_style_version:RESEARCH_OUTPUT_STYLE_VERSION,research_api:'available',gemini_key_present:Boolean(env.GEMINI_API_KEY),workers_ai_binding_present:Boolean(env.AI),gemini_test:'not-run'};
...
async function directGemini(request,env,origin){
 ...
 const routing=classifyResearchQuery(message);
 const research=await getResearchContext(message);
 let projectContext='';
 ...
 if(research.ref&&!research.used){
   return json({answer:'এই আয়াতের জন্য প্রকল্পের Research API থেকে যাচাইযোগ্য গবেষণা-রেকর্ড পাওয়া যায়নি। তাই আমি সাধারণ তাফসির বা অনুমান দিয়ে প্রকল্পের তথ্যের বিকল্প উত্তর দিচ্ছি না। আগে Research API সংযোগ/ডেটা যাচাই করতে হবে।',mode,language:'bn',provider:'research-api',brain_version:BRAIN_VERSION,research_output_style_version:RESEARCH_OUTPUT_STYLE_VERSION,intent:brain.type,research_routing:routing,research_context_used:false,research_context_ref:`${research.ref[0]}:${research.ref[1]}`,research_memory_used:memory.used},200,origin);
 }
 ...
 const systemInstruction=`${BRAIN_SYSTEM}\n\n${RESEARCH_OUTPUT_STYLE}`;
 const models=[env.GEMINI_MODEL||'gemini-2.5-flash','gemini-2.5-flash-lite'].filter((v,i,a)=>a.indexOf(v)===i);
 for(const model of models){try{const r=await fetch(`https://generativelanguage.googleapis.com/v1beta/models/${model}:generateContent`,{method:'POST',headers:{'Content-Type':'application/json','x-goog-api-key':env.GEMINI_API_KEY},body:JSON.stringify({systemInstruction:{parts:[{text:systemInstruction}]},contents:[{role:'user',parts:[{text:prompt}]}],generationConfig:{maxOutputTokens:4096,temperature:0.1}})});if(!r.ok)continue;const data=await r.json();const answer=data?.candidates?.[0]?.content?.parts?.map(p=>p?.text||'').join('').trim();if(answer){const recordProposal=buildResearchRecordProposal({message,answer,researchProvenance:research.provenance,memoryRecords:memory.records});return json({answer,mode,language:'bn',provider:model,brain_version:BRAIN_VERSION,research_output_style_version:RESEARCH_OUTPUT_STYLE_VERSION,intent:brain.type,research_routing:routing,research_context_used:research.used,research_context_ref:research.ref?`${research.ref[0]}:${research.ref[1]}`:null,research_memory_used:memory.used,research_memory_records:memory.records.map(r=>({record_id:r.record_id,version:r.version,status:r.status,ayah_id:r.scope?.ayah_id||null})),project_context_loaded:Boolean(projectContext),research_provenance:research.provenance,research_record_proposal:recordProposal},200,origin);}}catch{}}
 return null;
}

export default {async fetch(request,env,ctx){const origin=request.headers.get('Origin')||'';const gateway=await handleApiGateway(request,env,ctx);if(gateway)return gateway;const privateResearch=await handlePrivateResearch(request,env);if(privateResearch)return privateResearch;const rp=await readerProxy(request,env);if(rp)return rp;const d=await diagnostic(request,env);if(d)return d;if(request.method==='OPTIONS')return new Response(null,{status:204,headers:{'Access-Control-Allow-Origin':ALLOWED_ORIGINS.includes(origin)?origin:ALLOWED_ORIGINS[0],'Access-Control-Allow-Methods':'GET, POST, OPTIONS','Access-Control-Allow-Headers':'Content-Type','Content-Type':'application/json; charset=utf-8','Vary':'Origin'}});const research=await handleResearchApi(request);if(research)return research;const gemini=await directGemini(request,env,origin);if(gemini)return gemini;const response=await worker.fetch(request.clone(),env,ctx);if(response.status<500)return response;const recovery=await recoverChat(request.clone(),env);if(recovery)return recovery;return response;}};
