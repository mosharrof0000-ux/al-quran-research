/* আল-কুরআন গবেষণা — Worker Entry Wrapper v4.0
   ...
   নিরাপত্তা: Research API read-only context হিসেবে AI-কে দেওয়া হয়; raw data পরিবর্তন করা হয় না।
*/
...
async function getResearchContext(message){
 const ref=extractAyahReference(message);if(!ref)return '';
...
async function directGemini(request,env,origin){
 ...
 const context=await getResearchContext(message);
 const brain=buildBrainPrompt(message,context);
