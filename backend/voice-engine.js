...
export function applySmartPunctuation(segments, options = {}) {
  ...
  for (const segment of segments) {
    ...
    const isQuestion =
      ...
      (intonation === "rising" &&
        Number(segment.questionConfidence || 0) >= config.questionConfidence);
...
export function correctContextualGrammar(rawText, candidates = [], options = {}) {
  ...
  for (const candidate of sorted) {
    text = text.replace(
      tokenRegex(candidate.from),
      (_match, prefix) => prefix + candidate.to
    );
  }
...
export function sanitizePhoneticSpeech(audioStream, options = {}) {
  ...
  const rawText =
    typeof audioStream === "string"
      ? audioStream
      : audioStream && typeof audioStream.transcript === "string"
        ? audioStream.transcript
        : "";

  if (!rawText) {
    return { transcript: "", changed: false, mode: "text-sanitizer" };
  }

  let transcript = normalizeSpaces(rawText);
  const before = transcript;

  for (const [spoken, normalized] of Object.entries(aliases).sort(
    (a, b) => b[0].length - a[0].length
  )) {
    transcript = transcript.replace(
      tokenRegex(spoken),
      (_match, prefix) => prefix + normalized
    );
  }

  return {
    transcript,
    changed: transcript !== before,
    mode: "text-sanitizer",
  };
}
...
export function processVoiceTranscript({
  ...
  const original = normalizeSpaces(
  ...
  const corrected = correctContextualGrammar(
    punctuated,
    corrections,
    options.correction
  );
  const sanitized = sanitizePhoneticSpeech(corrected, {
