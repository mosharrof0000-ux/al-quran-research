/* Al-Quran Research — Isolated Project Agent Engine v1.1
   Identity-aware, task-traceable, isolated Project Agent.
   It can read project files and write only to agent/* branches.
...
function taskIdentity(message){
 const s=String(message||'').toLowerCase();
 let bn='সোহেল',slug='sohel',role='general';
 if(/icon|আইকন|svg/.test(s)){bn='শাহীন';slug='shaheen';role='icon';}
 else if(/reader|কুরআন|আয়াত|সূরা|quran/.test(s)){bn='সুমন';slug='sumon';role='reader';}
 else if(/chat|চ্যাট|voice|ভয়েস|ai/.test(s)){bn='শামীম';slug='shamim';role='ai';}
 else if(/font|text|টাইপ|ফন্ট/.test(s)){bn='রাকিব';slug='rakib';role='typography';}
 else if(/test|verify|browser|পরীক্ষা|যাচাই/.test(s)){bn='নাঈম';slug='naim';role='verification';}
 else if(/data|database|ডেটা|ডাটাবেজ/.test(s)){bn='আরিফ';slug='arif';role='data';}
 else if(/agent|system|engine|সিস্টেম/.test(s)){bn='ইমরান';slug='imran';role='agent-core';}
 const now=new Date();
 const stamp=now.toISOString().replace(/[-:TZ.]/g,'').slice(0,14);
 const rand=Math.random().toString(36).slice(2,7);
 const taskId='TASK-'+stamp+'-'+rand.toUpperCase();
 return {taskId,agentName:bn+'-'+stamp.slice(8)+'-'+rand,baseName:bn,slug,role,branch:'agent/'+slug+'-'+role+'-'+stamp+'-'+rand};
}
function ledgerText(meta,message){
 return '# Agent Work Record — '+meta.taskId+'\n\n'+
 '## Identity\n- Agent Name: '+meta.agentName+'\n- Agent Base Name: '+meta.baseName+'\n- Role: '+meta.role+'\n- Task ID: '+meta.taskId+'\n- Branch: '+meta.branch+'\n- Base Branch: '+(meta.baseBranch||'')+'\n- Created: '+new Date().toISOString()+'\n\n'+
 '## Request\n'+message+'\n\n'+
 '## Status\nRECEIVED → INSPECTING\n\n'+
 '## Rules\n- main পরিবর্তন নিষিদ্ধ\n- protected paths পরিবর্তন নিষিদ্ধ\n- কাজ অসম্পূর্ণ হলে handoff record বাধ্যতামূলক\n- merge/deploy এই engine-এর ক্ষমতার বাইরে\n\n'+
 '## Handoff\nঅন্য Agent নিলে এই Task ID, বর্তমান branch, completed/remaining কাজ এবং verification ফল অনুসরণ করতে হবে।\n';
}
const TOOLS=[
 ...
 {name:'project_create_branch',description:'Create an isolated agent/* branch. The current task branch is already created; use this only for a separate child task.',parameters:{type:'object',properties:{branch:{type:'string'},base:{type:'string'}},required:['branch']}},
...
async function gemini(env,history,meta){
 ...
 const system='তুমি আল-কুরআন গবেষণা প্রকল্পের Project Agent। বর্তমান পরিচয়: '+meta.agentName+'। Task ID: '+meta.taskId+'। Role: '+meta.role+'। Current branch: '+meta.branch+'। Base branch: '+meta.baseBranch+'। এই branch-এই কাজ করবে। প্রথমে governance ও প্রয়োজনীয় project files পড়বে। অনুমান করে rewrite করবে না। main-এ লিখবে না। .github/workflows, database, migrations, validation, quran_research.db এবং schema.sql পরিবর্তন করবে না। প্রয়োজনমতো inspect → plan → minimal edit → verify করবে। কাজ অসম্পূর্ণ হলে স্পষ্ট remaining কাজ এবং successor handoff লিখবে। কাজ শেষে কী পড়েছ, কী বদলেছ, branch, commit, verification, remaining, approval/deployment status বাংলায় জানাবে। merge/deploy দাবি করবে না।';
...
export default {async fetch(request,env){
 ...
 if(request.method==='GET')return json({ok:true,service:'al-quran-research-project-agent',version:env.AGENT_VERSION||'1.1.0',identity_registry:true,task_ledger:true,isolated:true,write_scope:'agent/* only',merge:false,deploy:false},200,origin);
 ...
 try{
  ...
  const meta=taskIdentity(message);meta.baseBranch=env.AGENT_BASE_BRANCH||'agent/project-engine-v1-isolated';
  const branchResult=await createBranch(env,{branch:meta.branch,base:meta.baseBranch});
  const ledgerPath='docs/agent-work-ledger/'+meta.taskId+'.md';
  const instructionPath=ledgerPath+'.instruction.md';
  const instruction='# Instruction — '+meta.taskId+'\n\nThis task record is an auditable Agent Work Ledger artifact. Preserve its identity, status, branch, handoff and verification history. Update it when task state changes. Governed by MASTER_INSTRUCTION.md, AI_ENTRY_PROTOCOL.md and PROJECT_AGENT_ENGINE_V1.md. Do not delete historical entries.\n';
  await writeFile(env,{path:ledgerPath,branch:meta.branch,content:ledgerText(meta,message),message:'Record '+meta.taskId+' agent identity and task intake'});
  await writeFile(env,{path:instructionPath,branch:meta.branch,content:instruction,message:'Add instruction for '+meta.taskId+' work record'});
  const result=await gemini(env,[{role:'user',parts:[{text:message}]}],meta);
  return json({ok:true,answer:result.answer,provider:result.model,agent_version:env.AGENT_VERSION||'1.1.0',task:meta,branch_created:branchResult,ledger:ledgerPath,isolated:true,merge:false,deploy:false},200,origin);
